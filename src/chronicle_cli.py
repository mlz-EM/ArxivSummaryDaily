"""Command-line workflow for Chronicle sitemap discovery and summaries."""
...
def _config() -> Dict[str, Any]:
    return {**DEFAULT_CONFIG, **getattr(settings, "CHRONICLE_CONFIG", {})}
...
def _build_parser() -> argparse.ArgumentParser:
    config = _config()
    parser = argparse.ArgumentParser(description="Chronicle job discovery and summary pipeline")
    parser.add_argument("--output-dir", default=getattr(settings, "OUTPUT_DIR", "data"))
    subparsers = parser.add_subparsers(dest="command", required=True)

    scan = subparsers.add_parser("scan", help="discover new jobs from Chronicle's sitemap")
    ...
    summarize = subparsers.add_parser("summarize", help="summarize all queued Chronicle jobs")
...
def _paths(args: argparse.Namespace) -> tuple[Path, Path]:
    pending = Path(args.pending_file or _default_path(args.output_dir, "chronicleJobsPending.json"))
    state = Path(args.state_file or _default_path(args.output_dir, "chronicleScanState.json"))
    return pending, state


def _scan(args: argparse.Namespace) -> int:
    if os.getenv("CHRONICLE_ENABLE_LIVE", "").lower() not in {"1", "true", "yes"}:
        raise RuntimeError(
            "live Chronicle fetching is disabled; set CHRONICLE_ENABLE_LIVE=true after confirming authorization"
        )
    pending_path, state_path = _paths(args)
    store = ChronicleStore(pending_path, state_path)
    if args.mode == "daily" and not store.state.get("bootstrapComplete"):
        raise RuntimeError("initial Chronicle bootstrap is incomplete; run scan --mode bootstrap first")
    ...
    # A bootstrap remains incomplete until every current sitemap job has been
    # classified. Daily runs retain failures for retry but should still allow
    # successfully queued jobs to reach the summarizer in the same workflow.
...
def _set_header(path: Path, model: str) -> None:
    ...
    payload["header"] = {
        ...
        "notes": [
            ...
            "Current jobs are discovered from Chronicle's public job sitemap; only newly seen listings are summarized.",
        ],
    }
    ChronicleStore._atomic_write(path, payload)
...
def _summarize(args: argparse.Namespace) -> int:
    ...
    output_path = Path(args.output_file or _default_path(args.output_dir, "chronicleJobsSummary.json"))
    llm_config = settings.LLM_CONFIG
    api_key = str(llm_config.get("api_key") or "").strip()
    if pending and (not api_key or api_key == "YOUR_API_HERE"):
        raise RuntimeError("LLM_API_KEY is required to summarize pending Chronicle jobs")
    ...
    if not success:
        print("Chronicle summary was incomplete; queued records were retained for retry.")
        return 1

    store.remove_pending(int(job["id"]) for job in pending)
    store.save()
    store.remove_pending_file_if_empty()
    print(f"Summarized {len(pending)} Chronicle jobs and removed them from the pending queue.")
    return 0
